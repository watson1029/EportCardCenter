...
{
    public partial class PrivateOnlineVoting : System.Web.UI.Page
    {
        ...
        protected void submit_Click(object sender, EventArgs e)
        {
            ...
            switch (dlStatistics.Value)
            {
                case "Statistics5":
                    ...
                    filename = string.Format("{0}-{1}-{2}.xls", begin.ToString("yyyyMMdd"), end.ToString("yyyyMMdd"), "业务工单详细信息统计（所有）");
                    break;
                case "Statistics6":
                    ds = new DataAccess.RunProcedure().Statistics6(begin, end);
                    ds.Tables[0].Columns.Add("关区代码");
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        ds.Tables[0].Rows[i][8] = GetCustomCode(ds.Tables[0].Rows[i][3].ToString());
                    filename = string.Format("{0}-{1}-{2}.xls", begin.ToString("yyyyMMdd"), end.ToString("yyyyMMdd"), "业务工单详细信息统计（其他关区）");
                    break;
            }
