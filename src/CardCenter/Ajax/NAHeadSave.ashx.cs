...
namespace CardCenter.Ajax
{
    ...
    /// NAHeadSave 的摘要说明
    /// </summary>
    public class NAHeadSave : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                {
                    DataSet view = procedure.JobView(jobID, "NewlyAddedList");
                ...
                if (flat)
                {
                    ...
                    job.JobID = string.IsNullOrEmpty(jobID) ? "NA-" + CardCenter.DataAccess.CardCenterHelper.GetCardNo() : jobID;
                    job.JobType = "NA";
                    job.CreateTime = time;
                    ...
                    DataSet list = new DataAccess.NewlyAddedList().GetList("JobID='" + jobID + "' and IsDelete=0");
                    ...
                    if (context.Request.Form["type"] == "complete")
                    {
                        ...
                        //判断是否添加工单项
                        ds = procedure.JobView(jobID, "NewlyAddedList");
                    ...
                    flow.FlowID = context.Request.Form["hSaveType"] == "complete" ? int.Parse(new CardCenter.DataAccess.Flow().GetList("JobType='NA' and FlowOrder=2").Tables[0].Rows[0]["FlowStatus"].ToString()) : 1;
                    flow.SubmitDate = time;
                    flow.SubmitUser = PageBase.CommonObject.LoginUserInfo.companyId;
                    flow.Content = context.Request.Form["hSaveType"] == "complete" ? PageBase.CommonObject.LoginCompanyInfo.FULL_NAME + "提交工单." : PageBase.CommonObject.LoginCompanyInfo.FULL_NAME + "暂存数据.";
                    flow.IsDelete = false;
                    flow.Remark = string.Empty;

                    //提交数据到后台存储过程事务
                    DataAccess.TranHelper.BeginTran();
                    DataAccess.NADataAccess na = new DataAccess.NADataAccess();
                    na.InsertHeadData(job, fileList, flow);
                    if (context.Request.Form["hSaveType"] == "complete")
                        na.InsertHandleList(job);
                    try
                    {
