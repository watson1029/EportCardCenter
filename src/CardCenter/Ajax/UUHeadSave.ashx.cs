...
namespace CardCenter.Ajax
{
    ...
    /// UUHeadSave 的摘要说明
    /// </summary>
    public class UUHeadSave : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                {
                    DataSet view = procedure.JobView(jobID, "UpdateUnlockList");
                ...
                if (flat)
                {
                    ...
                    job.JobID = string.IsNullOrEmpty(jobID) ? "UU-" + CardCenter.DataAccess.CardCenterHelper.GetCardNo() : jobID;
                    job.JobType = "UU";
                    job.CreateTime = time;
                    ...
                    DataSet ds = procedure.SelectFileTypeByJobType("UU");
                    ...
                    DataSet list = new DataAccess.UpdateUnlockList().GetList("JobID='" + jobID + "' and IsDelete=0");
                    ...
                    if (context.Request.Form["type"] == "complete")
                    {
                        ...
                        //判断是否添加工单项
                        ds = procedure.JobView(jobID, "UpdateUnlockList");
                    ...
                    flow.FlowID = context.Request.Form["hSaveType"] == "add" ? 1 : int.Parse(new CardCenter.DataAccess.Flow().GetList("JobType='UU' and FlowOrder=2").Tables[0].Rows[0]["FlowStatus"].ToString());
                    ...
                    DataAccess.UUDataAccess uu = new DataAccess.UUDataAccess();
                    uu.InsertHeadData(job, fileList, flow);
                    if (context.Request.Form["hSaveType"] == "complete")
                        uu.InsertHandleList(job);
                    try
                    {
