<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Sale.aspx.cs" Inherits="CardCenter.ApplyForm.Sale" %>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function () {
            ...
            function onFinishCallback() {
                if (JobHeadSaveVerifi() && JobHeadInvoiceVerifi()) {
                    jQuery.ajax({
                        type: "POST",
                        dataType: "text",
                        url: "../Ajax/Commodity/VerifiCommodity.ashx",
                        data: "jobID=" + jQuery("#hJobID").val() + "&jobList=" + jQuery("#hJobList").val(),
                ...
                switch (stepObj.attr("id")) {
                    ...
                    case "tab3":
                        if ($("input:radio[name='expressflat']:checked").val() == "上门领取") {
                            return JobHeadInvoiceVerifi();
                        }
                        else {
                            if (JobHeadStep3Verifi() && JobHeadInvoiceVerifi())
                                return true;
                            else
                                return false;
                        }
            ...
            jQuery("input:radio").change(function () {
                if (jQuery("#expressTrue").attr("checked") == "checked") {
                    jQuery(".stepContainer").css("height", "508px");
                    jQuery("#expressMsg").hide();
                    jQuery("#divConsignee").show();
                }
                else {
                    jQuery(".stepContainer").css("height", "434px");
            ...
            jQuery("#agentName").change(function () {
                ...
                jQuery("#txtInvoiceName").val(jQuery(this).val());
            });

            jQuery("#agentPhone").change(function () {
                jQuery("#consigneePhone").val(jQuery(this).val());
                jQuery("#txtInvoicePhone").val(jQuery(this).val());
            });

            jQuery(".checkJobList").click(function () {
                var jobList = "";
                jQuery("#jobList").children().each(function () {
                    if (jQuery(this).find("span").attr("class") == "checked")
                        jobList += jQuery(this).attr("id") + ';';
                });
                jQuery("#hJobList").val(jobList);
                GetCardNumMsg(jobList)
            });

            if(jQuery("hJobID").val() != "")
                GetCardNumMsg(jQuery("#hJobList").val());
        });

        function GetCardNumMsg(jobList)
        {
            jQuery.ajax({
                type: "POST",
                dataType: "text",
                url: "../Ajax/Commodity/SelectCardNum.ashx",
                data: "jobID=" + jQuery("#hJobID").val() + "&jobList=" + jobList,
                success: function (data) {
                    jQuery("#cardMsg").html(data);
                },
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    jAlert("网络出错，请稍后再试" + errorThrown, "提示");
                }
                    ...
                    if (data.error == "") {
                        if (type == "complete") {
                            ...
                            //    }
                                ...
                                //window.open("../Manager/Pay.aspx?jobID=" + '<%= jobID %>');
                            });
                        }
                        else {
                            location.href = "SaleList.aspx?jobID=" + data.msg;
                        }
                    }
                    else {
                        if (type == "complete") {
                        ...
                        jQuery("#btnAddList").removeAttr("disabled");
    ...
    <div class="centercontent">
        <div id="contentwrapper" class="contentwrapper">
            ...
            <div id="tabbed" class="subcontent">
                ...
                <input type="hidden" runat="server" id="hJobList" />
                    ...
                    <ul class="tabbedmenu">
                        ...
                        <li>
                            <a id="tab3" href="#wiz1step2_3">
                                <span class="label">第三步: 快递信息</span>
                            </a>
                        </li>
                        <li>
                            <a id="tab4" href="#wiz1step2_4">
                                <span class="label">第四步: 安全产品信息</span>
                            </a>
                        </li>
...
当您按照页面提示阅读并勾选“我已经阅读并同意用户许可协议”，即表示您已充分阅读、理解并同意接受本协议的全部内容的约束。您承诺接受并遵守本协议的约定，届时您不应以未阅读本协议的内容或者未获得我中心对您问询的解答等理由，主张本协议无效，或要求撤销本协议，具体协议如下：
    ...
    联系咨询电话：020-83939000。
                    ...
                    <div id="wiz1step2_3">
                        ...
                        <p>
                            <span class="formwrapper">
                                <input type="radio" name="expressflat" checked="true" runat="server" id="expressTrue"  value="快递"/>快递&nbsp;&nbsp;
                            	<input type="radio" name="expressflat"  runat="server" id="expresssFalse" value="上门领取"/>上门领取
                                <span style="font-weight:bold;color:red;margin-left:50px;">注意：安全产品领取方式、快递收货人资料及快递地址提交后不得更改，请确认所录入资料、地址准确无误后再予以提交！</span>
                            </span>
                        </p>
                        <p id="expressMsg">
                            <span class="formwrapper">
                                领取地址：广州市天河区珠江新城华利路61号广州市政务服务中心4楼&nbsp;&nbsp;&nbsp;联系电话：020-83939000-2
                            </span>
                        </p>
                        ...
                        <p>
                            <span class="formwrapper">
                                东方口岸科技有限公司发票从北京统一寄送，寄送时间约10-15个工作日<br />
                                南方口岸科技有限公司发票为电子发票，10-15个工作日后发送至电子邮箱。
                            </span>
                        </p>
                        <table class="table">
                            <tr>
                                ...
                                <td class="content1">
                                    <input type="text" id="txtInvoiceName" class="longinput" runat="server"/>
                                </td>
                                <td class="title1">
                                    发票收件人手机
                                </td>
                                <td class="content1">
                                    <input type="text" id="txtInvoicePhone" class="longinput" runat="server"/>
                                </td>
                            </tr>
                            <tr>
                                <td class="title1">
                                ...
                                <td class="content1">
                                    <input type="text" id="txtInvoiceAddress" class="longinput" runat="server"/>
                                </td>
                                <td class="title1">
                                    发票收件地址邮政编码
                                </td>
                                <td class="content1">
                                    <input type="text" id="txtInvoicePostCode" class="longinput" runat="server"/>
                                </td>
                            </tr>
                            <tr>
                                <td class="title1">
                                ...
                                <td class="content1">
                                    <input type="text" id="txtInvoiceCode" class="longinput" runat="server"/>
                                </td>
                                <td class="title1">
                                    发票电子邮箱
                                </td>
                                <td class="content1">
                                    <input type="text" id="txtInvoiceEmail" class="longinput" runat="server"/>
                                </td>
                            </tr>
                        </table>
                    </div><!--#wiz1step2_3-->
                    <div id="wiz1step2_4">
                        <div class="pageheader notab">
                            <h1 class="pagetitle">第四步: 安全产品信息</h1>
                            <span class="pagedesc">请选择以下工单进行绑定。根据您的选择，您需要添加：<span id="cardMsg">0张法人卡，0张操作员IC卡，0张操作员IKEY，0张报关员卡。</span></span>
                        </div>
                        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
                            ...
                            <tbody id="jobList">
                                <asp:Repeater id="repeater" runat="server" OnItemDataBound="repeater_ItemDataBound">
                                    <ItemTemplate>
                                        <tr id="<%# Eval("JobID") %>">
                                            <td class="con0"><input type="checkbox" class="checkJobList" id="cbIsBind" runat="server"/></td>
                                            ...
                                            <td class="con0"><a target="_blank" href='../Manager/View.aspx?jobID=<%# Eval("JobID") %>&jobType=<%# Eval("JobType") %>'>查看</a></td>
                                        </tr>
                                    </ItemTemplate>
                                </asp:Repeater>
                            </tbody>
                        ...
                        <p class="stdformbutton">
                        	<button id="btnAddList" class="submit radius2" runat="server" onclick="onSave('add');return false;">添加产品信息</button>
                        </p>
                        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
                            <colgroup>
                                <col class="con0" />
                                ...
                                <asp:Repeater ID="repeater1" runat="server">
                                    <ItemTemplate>
                                        <tr id="<%# Eval("ListID") %>">
                                            ...
                                            <td class="con1"><%# Eval("CommodityName") %></td>
                                            <td class="con0"><%# Eval("Num") %></td>
                                            <td class="con1"><a href="SaleList.aspx?jobID=<%=jobID %>&listID=<%# Eval("ListID") %>">修改</a>&nbsp;&nbsp;<a href="#" onclick="deleteSLList(this)">删除</a></td>
                                        </tr>
                                    </ItemTemplate>
                                </asp:Repeater>
                            </tbody>
