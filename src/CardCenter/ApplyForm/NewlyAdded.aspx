<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NewlyAdded.aspx.cs" Inherits="CardCenter.ApplyForm.NewlyAdded" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function onSave(type) {
            jQuery("#btnAddList1").attr("disabled", "disabled");
            jQuery("#btnAddList1").attr("class", "submitdis radius2");
            jQuery("#btnAddList2").attr("disabled", "disabled");
            jQuery("#btnAddList2").attr("class", "submitdis radius2");
            jQuery("#hSaveType").val(type);
            jQuery.ajax({
                type: "POST",
                dataType: "json",
                url: "../Ajax/NAHeadSave.ashx",
                data: $('#updateForm').serialize() + "&type=" + type,
                success: function (data) {
                    if (data.error == "") {
                        if (type == "complete") {
                            jAlert("保存成功，确定后将跳转至安全产品页面！\n请根据您提交的工单，选购相应的安全产品！\n请使用法人卡登录中国电子口岸执法系统录入新增的操作员信息！", "提示", function () {
                            ...
                            //    }
                                location.href = "Sale.aspx?bindid=" + jQuery("#hJobID").val() + "&name=" + jQuery("#agentName").val() + "&phone=" + jQuery("#agentPhone").val();
                                //window.open("http://web.chinaport.gov.cn/rasuc/");
                                //window.open("../Manager/Pay.aspx?jobID=" + '<%= jobID %>');
                            });
                        }
                        if (type == "addFR") {
                            location.href = "NewlyAddedListFR.aspx?jobID=" + data.msg;
                        }
                        if (type == "addQT") {
                            location.href = "NewlyAddedListQT.aspx?jobID=" + data.msg;
                        }
                    }
                    else {
                        if (type == "complete") {
                        ...
                        jQuery("#btnAddList1").removeAttr("disabled");
                        jQuery("#btnAddList1").attr("class", "submit radius2");
                        jQuery("#btnAddList2").removeAttr("disabled");
                        jQuery("#btnAddList2").attr("class", "submit radius2");
                    }
                },
                    ...
                    <ul class="tabbedmenu">
                        ...
                        <li>
                            <a id="tab4" href="#wiz1step2_4">
                                <span class="label">第三步: 持卡人信息</span>
                            </a>
                        </li>
                        ...
                        <% { %>
                            <li>
                                <a id="tab5" href="#wiz1step2_5">
                                    <span class="label">第四步: 随附资料</span>
                                </a>
                            </li>
                        <% } %>
                    </ul>
    ...
    联系咨询电话：020-83939000。
                        ...
                        <p>
                            <span class="formwrapper">
                            	<input type="checkbox" id="checkAgree" onclick="agree();" checked="checked" /> 我已经阅读并同意用户许可协议.<br />
                            </span>
                        </p>
                    </div><!--#wiz1step2_1-->
                    ...
                    <div id="wiz1step2_4">
                        ...
                        <p class="stdformbutton">
                            <button id="btnAddList1" class="submit radius2" runat="server" onclick="onSave('addFR');return false;">新增法人卡信息</button>
                        	<button id="btnAddList2" class="submit radius2" runat="server" onclick="onSave('addQT');return false;">新增其他卡信息</button>
                        </p>
                        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="head1">持卡人手机号</th>
                                    <th class="head0">证件类型</th>
                                    <th class="head1">证件号码</th>
                                ...
                                <asp:Repeater ID="repeater1" runat="server">
                                    <ItemTemplate>
                                        <tr id="<%# Eval("ListID") %>">
                                            ...
                                            <td class="con1"><%# Eval("CardholderPhone") %></td>
                                            <td class="con0"><%# Eval("CardholderIdentificationType") %></td>
                                            <td class="con1"><%# Eval("CardholderIdentificationNum") %></td>
                                            <td class="con0"><a href="<%# Eval("JobTypeID").ToString() == "NA001" ? "NewlyAddedListFR" : Eval("JobTypeID").ToString() == "NA002" ? "NewlyAddedListFREx" : "NewlyAddedListQT" %>.aspx?jobID=<%=jobID %>&listID=<%# Eval("ListID") %>">修改</a>&nbsp;&nbsp;<a href="#" onclick="deleteNAList(this)">删除</a></td>
                                        </tr>
                                    </ItemTemplate>
                                </asp:Repeater>
                            </tbody>
