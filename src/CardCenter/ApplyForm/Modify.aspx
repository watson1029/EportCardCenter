<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Modify.aspx.cs" Inherits="CardCenter.ApplyForm.Modify" %>
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            jQuery("input:radio").change(function () {
                if (jQuery("#expressTrue").attr("checked") == "checked") {
                    jQuery(".stepContainer").css("height", "405px");
                    jQuery("#expressMsg").hide();
                    jQuery("#divConsignee").show();
                }
                else {
                    jQuery(".stepContainer").css("height", "297px");
        ...
        function onSave(type) {
            ...
            jQuery.ajax({
                type: "POST",
                dataType: "json",
                url: "../Ajax/MDHeadSave.ashx",
                data: $('#updateForm').serialize() + "&type=" + type,
                success: function (data) {
                    if (data.error == "") {
                        if (type == "complete") {
                            jAlert("保存成功！\n请使用法人卡登录中国电子口岸执法系统进行变更信息录入！", "提示", function () {
                            ...
                            location.href = "ModifyList.aspx?jobID=" + data.msg;
                        }
                    }
                    else {
                        if (type == "complete") {
...
当您按照页面提示阅读并勾选“我已经阅读并同意用户许可协议”，即表示您已充分阅读、理解并同意接受本协议的全部内容的约束。您承诺接受并遵守本协议的约定，届时您不应以未阅读本协议的内容或者未获得我中心对您问询的解答等理由，主张本协议无效，或要求撤销本协议，具体协议如下：
    ...
    （三）企业须在A4规格纸上打印完整的回邮信息和办卡联系邮箱（
                                    回邮信息包括：收件单位名称、收件人姓名、邮寄地址、联系电话、联系邮箱等内容）并加盖公章进行确认，如因信息不完整而导致一切后果由企业自行承担。
    ...
    联系咨询电话：020-83939000。
                    ...
                    <div id="wiz1step2_3">
                        ...
                        <p>
                            <span class="formwrapper1">
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;电子口岸卡变更需分别在本系统提交变更业务工单申请，以及访问中国电子口岸执法系统提交变更内容，并等待数据分中心进行资料确认。
                            </span>
                            <span class="formwrapper1">
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;如变更涉及以下内容：（1）企业名称变更，（2）法人名称变更（法人卡变更时），（3）持卡人姓名变更（操作员IC卡，IKey变更时），还必须将电子口岸卡带至或快递至制卡点进行写卡操作。企业可按照自身情况在收到短信提示后，前往就近制卡点现场办理写卡，或将卡快递至广州制卡点写卡。
                            </span>
                            <span class="formwrapper1">
                            ...
                            <tr>
                                <td colspan="4">
                                    <span style="display:block;margin:auto 50px;">
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;快递办理需在本系统完成完整申请及资料提交、收到短信提示后，将电子口岸卡快递至广州市政务服务中心制卡点写卡<a href="../Express.aspx" target="_blank"><strong>（点此查看快递单填写要求）</strong></a>，完成写卡操作后电子口岸卡将快递寄回企业，请企业务必仔细填写下方快递寄回收货人姓名、手机号及收货地址。
                                    </span>
                                </td>
                            </tr>
                            <tr>
                    ...
                    <div id="wiz1step2_4">
                        ...
                        <p class="stdformbutton">
                        	<button id="btnAddList" class="submit radius2" runat="server" onclick="onSave('add');return false;">变更电子口岸卡信息</button>
                        </p>
                                ...
                                <asp:Repeater ID="repeater1" runat="server">
                                    <ItemTemplate>
                                        <tr id="<%# Eval("ListID") %>">
                                            ...
                                            <td class="con0"><a href="ModifyList.aspx?jobID=<%=jobID %>&listID=<%# Eval("ListID") %>">修改</a>&nbsp;&nbsp;<a href="#" onclick="deleteMDList(this)">删除</a></td>
                                        </tr>
                                    </ItemTemplate>
                                </asp:Repeater>
                            </tbody>
