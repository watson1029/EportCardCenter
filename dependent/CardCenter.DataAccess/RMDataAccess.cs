...
namespace CardCenter.DataAccess
{
    public class RMDataAccess
    {
        ...
        public void InsertListData(Entity.ReMakeList rm)
        {
            DataAccess.ReMakeList daRM = new ReMakeList();
            string sqlRM = string.Empty;
            if (daRM.Exists(rm.ListID))
                daRM.Update(rm);
            else
                daRM.Add(rm);
        }
